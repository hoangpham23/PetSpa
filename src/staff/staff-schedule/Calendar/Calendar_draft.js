...
const Calendar_draft = ({ onSelectDate }) => {
  ...
  const handleClick = (index) => {
    ...
    onSelectDate(date); // Gọi hàm callback với ngày được chọn
  };
      ...
      <div className={style.week_calendar}>
        ...
        {Array.from({ length: 7 }, (_, index) => {
          ...
          return (
            <div
              key={index}
              className={`${style.day} ${date.toDateString() === selectedDate.toDateString() ? style.active : ""}`}
